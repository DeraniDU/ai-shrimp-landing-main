...
while True:
    for i in range(1, 4):
        pond_id = str(i)
        data = generate_data(pond_id)
        try:
            response = requests.post(API_URL, json=data)
            print(f"Sent Pond {pond_id}: {data} | Response: {response.status_code}")
        except Exception as e:
            print(f"Error sending data for Pond {pond_id}: {e}")
